...
The `@lib/apollo` package exports a utility function called `createSupergraphManager` that fetches
the Supergraph schema from the Hive's CDN, and also a
...
import { ApolloServer } from '@apollo/server'
import { startStandaloneServer } from '@apollo/server/standalone'
import { createSupergraphManager, useHive } from '@lib/apollo'
...
- [`@lib/apollo` source code](https://github.com/graphql-hive/platform/tree/main/packages/libraries/apollo)
