...
The `@lib/core` package exports a utility function called `createServicesFetcher` that can be used
to fetch services definition from Hive's CDN. You can use it to create a GraphQL schema from the all
services schemas published to Hive.

<Callout type="info">
  The `createServicesFetcher` function is a part of the `@lib/core` package, and it can be used with
  any GraphQL server runtime. You may use it with Apollo Server, GraphQL Yoga, or any other library
  to implement a custom GraphQL gateway.
</Callout>
...
import { createServicesFetcher } from '@lib/core'
