...
[Docker image](https://github.com/graphql-hive/platform/pkgs/container/composition-federation-2)
...
docker run -p 3069:3069 -e SECRET="MY_SECRET_HERE" \
  ghcr.io/graphql-hive/composition-federation-2
```
...
```sh npm2yarn
npm i -D @lib/external-composition
```

<Callout type="info">
  The `@lib/external-composition` library provides a thin `compose` function to bring
...
import { compose, signatureHeaderName, verifyRequest } from '@lib/external-composition'
...
<Callout type="info">
  ...
  library](https://github.com/graphql-hive/platform/blob/4af001a7102b255bd8ceb8e3f01722faceb8f2f3/packages/services/external-composition/federation-2).
</Callout>
...
[Publish a GraphQL schema](/docs/schema-registry#publish-a-schema) or perform a
[GraphQL schema check](/docs/schema-registry#check-a-schema) to validate your setup.
