...
The official JavaScript Hive Client (`@lib/core`) collects executed operations and sends them in
batches (as a single report, when a buffer is full or every few seconds) over HTTP.
